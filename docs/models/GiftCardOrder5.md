# Org.OpenAPITools.Model.GiftCardOrder5
...
**Revision** | **string** | Revision number, which increments by 1 each time the Order is updated. To prevent conflicting changes, the current revision must be passed when updating the Order.  Ignored when creating a Order. | [readonly] 
